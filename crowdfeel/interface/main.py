...
from crowdfeel.TweepyAPI.twitterapi import hashtag_query,handle_query,geo_query,mention_query
...
def predloc_emo(model,distance,location) -> np.ndarray:
    ...
    x_test = tokenizer(text=X_processed,add_special_tokens=True,max_length=36,truncation=True,padding=True,
...
def predacc_emo(model,account) -> dict:

    # Extract data from the tweet API filtered by account
    X_pred=handle_query(str(account))
...
#Account mention
def predaccmen(model,account) -> np.ndarray:
    '''
    Extract data from Twitter based on an account
    '''
    #Extract data from the tweet API filterd by account name
    X_pred=mention_query(str(account))
...
def predaccmen_emo(model,account) -> dict:
